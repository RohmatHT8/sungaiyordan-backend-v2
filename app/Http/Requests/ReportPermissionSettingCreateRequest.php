...
class ReportPermissionSettingCreateRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        $rules = Helper::addTransactionAttributeRule([
            ...
            'role_ids.*' => ['required','distinct','exists:roles,id,deleted_at,NULL,need_approval,0',
                new ValidRoleBranchReportPermission('role_id','branch_id',$this->request->all('branch_ids'))
            ],
            'branch_ids' => 'required|array',
            'branch_ids.*' => ['required','distinct','exists:branches,id,deleted_at,NULL,need_approval,0', new OwnBranch,
                new ValidRoleBranchReportPermission('branch_id','role_id',$this->request->all('role_ids'))
            ],
